#!/usr/bin/env python3
# ProTracker MOD Choral Generator (v2.0.3)
...
def run_gui():
    ...
    root.title("ProTracker MOD Choral Generator (v2.0.3)")
    ...
    # Keep platform/theme button background; only tweak disabled foreground for clarity
...
# Advanced options (kept in a compact panel to avoid clutter)
    ...
    # Make Pattern Preview read-only and render content on selection.
    patt_txt.config(state="disabled")

    def _set_patt_text(s: str):
        try:
            patt_txt.config(state="normal")
            patt_txt.delete("1.0", "end")
            patt_txt.insert("1.0", s)
        finally:
            try:
                patt_txt.config(state="disabled")
            except Exception:
    ...
    def update_pattern_preview(event=None):
        """Render the selected pattern in ProTracker-ish monospace text."""
        try:
            song = last_song
            if song is None or not getattr(song, "patterns", None):
                _set_patt_text(tr("No song loaded yet. Generate a song first."))
                return
            pats = getattr(song, "patterns", []) or []
            try:
                p_idx = int(str(patt_sel_var.get() or "0").strip() or "0")
            except Exception:
                p_idx = 0
            p_idx = max(0, min(p_idx, len(pats) - 1))
            patt_sel_var.set(str(p_idx))
            pat = pats[p_idx]

            # Where in the order list this pattern appears (useful for debugging).
            occ = []
            try:
                for i, pp in enumerate(getattr(song, "order", []) or []):
                    if int(pp) == int(p_idx):
                        occ.append(i)
            except Exception:
                occ = []
            occ_txt = (", ".join(str(i) for i in occ[:24]) + ("..." if len(occ) > 24 else "")) if occ else "-"

            lines = []
            lines.append(f"PATTERN {p_idx}    used in order positions: {occ_txt}\n")
            lines.append("ROW  | CH1           | CH2           | CH3           | CH4\n")
            lines.append("-----+---------------+---------------+---------------+---------------\n")

            # Each row is [ (note,sample,effect,param) * 4 ]
            n_rows = min(64, len(pat) if hasattr(pat, "__len__") else 64)
            for r in range(n_rows):
                ...
                for ch in range(4):
                    try:
                        ...
                        cell = (None, 0, 0, 0)
                    try:
                        cells.append(_cell_to_text(cell))
                    except Exception:
                        cells.append("--- -- ---")
                lines.append(f"{r:02X}   | {cells[0]:<13} | {cells[1]:<13} | {cells[2]:<13} | {cells[3]:<13}\n")

            _set_patt_text("".join(lines))
        except Exception as e:
            _set_patt_text(tr("Pattern preview error: ") + str(e))
